...
Все современные процессоры поддерживают SIMD-инструкции, позволяющие выполнять операции над векторами чисел параллельно.

#### Причины эффективности подобных инструкций
Высокая эффективность достигается за счёт реализации на аппаратном уровне. Для поддержки подобных инструкций:
- Создаются специальные блоки векторных регистров 
    - **SSE**: 128 бит(XMM)
    - **AVX**: 256 бит(YMM)
    - **AVX-512**: 512 бит(ZMM)
Процессор воспринимает их не как число, а как набор данных.
- **ALU** SIMD-процессоров "составные" для того, чтобы работа происходила как с набором чисел, а не как с одним числом. В процессор встраиваются "перегородки", которые помогают разрывать цепи переноса между битами. При работе с большими числами, а не с наборами чисел "перегородки" могут отключаться. Это получило название **Data Path Splitting**.
- Отдельный **порт исполнения**(Execution Port), который соединен с векторным модулем. 
- Для перестановок битов в векторных регистрах существует довольно сложный аппаратный блок **Shuffle Unit**
- Для избежания создания "бутылочно горлышка" при передаче данных создаются расширенные шины для быстрой передачи векторов данных (например между L1 кешем и регистрами)

Вычисления для построения множества Мандельброта крайне ресурсоемкии и fps его отображения крайне низкий при компиляции даже с опцией -O3 достигает лишь ~7-8. Поэтому возникает необходимость оптимизации. 

Основной идеей на которой основана оптимизация является полноценное использование возможностей по работе с векторами данных. Для этого в современных процессорах существует отдельный аппаратный блок. За счёт этого эти инструкции являются крайне эффективными и предпочтительными при работе с большими объемами данных. Также при обработке данных порциями частично уйдут накладные расходы например на проверку условий (то есть произойдет **развертка цикла**). При таком объеме вычислений влияние данных оптимизаций тяжело переоценить.
