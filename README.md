...
- флаг **--no_graphics** для отключение визуализации (используется при бенчмарках)
...
#### Основная идея 
Ключевая оптимизация основана на использование **SIMD (Single Instruction, Multiple Data)** - подход при котором одна инструкция обрабатывает сразу несколько элементов данных.
Современные процессоры поддерживают SIMD-инструкции (я использовал **AVX-512**), позволяющие выполнять операции над векторами чисел параллельно.
...
- Отдельный **порт исполнения**(Execution Port), который соединен с векторным модулем.
...
- Расширение шины передачи данных между кешем и регистрами уменьшают задержки при работе с большими объёмами данных
...
int curComplexCmp[UNWRAP_NUMBER] = {0};

for (int i = 0; i < UNWRAP_NUMBER; i++) {
    if (curComplexSquare[i] < MAX_COMPLEX_NUM_SIZE)
        curComplexCmp[i] = 1;
}
```
...
Для количественной оценки ускорения проведен полноценный тест производительности(см. далее)
